...
// Conexiones:
//- TX (Pin 1) → RX del ESP32 (RX2)
//- RX (Pin 0) ← TX del Arduino UNO 1
//- GND común
...
void setup() {
  ...
  Serial.println("[INFO] Direccion: Node-5 (Arduino UNO 2)");
  Serial.println("[INFO] Siguiente: Node-1 (ESP32)");
  Serial.println("[INFO] Anterior: Node-4 (Arduino UNO 1)");
  Serial.println();
  Serial.println("-------------------------------------");
}
...
bool waitForToken() {
  printTime();
  Serial.println(" [Node-5] Esperando token...");
    ...
    if (Serial.available()) {
      ...
      if (index == 0 && byte == TOKEN_BYTE) {
        ...
        if (index == 5) {
          ...
          printTime();
          Serial.println(" [Node-5] <== TOKEN RECIBIDO desde Node-4");
...
void processToken() {
  printTime();
  Serial.println(" [Node-5] Procesando token...");
  
  uint16_t processTime = 105 + (sequence * 3);
  delay(processTime);
...
void sendToken() {
  ...
  printTime();
  Serial.println(" [Node-5] ==> TOKEN ENVIADO a Node-1");
  Serial.println("                       Estado: OK");
  Serial.println("                       [ANILLO COMPLETADO]");
...
void printStatistics() {
  Serial.println();
  Serial.println("=====================================");
  Serial.println("ESTADISTICAS - Node-5 (Arduino UNO 2)");
  Serial.println("=====================================");
  ...
  Serial.print("Rotaciones completadas: "); Serial.println(rotations);
  Serial.println("Tiempo por rotacion: ~2.5 segundos");
  Serial.println("=====================================");
}
